...
impl Step for Frank {
    ...
    fn step(&mut self, config: &mut Config) {
        ...
        let output = Command::new("runuser").args(["-u", &config.user, "--", &format!("{}/fd_wksp_ctl", bin.to_str().unwrap()),
            "new", &workspace, &config.workspace.page_count.to_string(), &config.workspace.page_size.to_string(), &config.affinity.to_string(), "0600"]).output().unwrap();
        if !output.status.success() {
        ...
        let main_cnc = run!(bin, "fd_tango_ctl", ["new-cnc", workspace, 0, "tic", config.cnc_app_size]);
        run!(bin, "fd_pod_ctl", ["insert", pod, "cstr", format!("{name}.main.cnc"), main_cnc]);

        // Pack tiles
        let cnc = run!(bin, "fd_tango_ctl", ["new-cnc", workspace, 0, "tic", config.cnc_app_size]);
        ...
        for i in 0..config.tiles.verify.count {
            ...
            let cnc = run!(bin, "fd_tango_ctl", ["new-cnc", workspace, 2, "tic", config.cnc_app_size]);
        ...
        let output = if config.netns.enabled {
            run!(bin, "fd_xdp_ctl", ["hook-iface", "--netns", format!("/var/run/netns/{}", &config.tiles.quic.interface), name, config.tiles.quic.interface, config.tiles.quic.xdp_mode]);
            let output = Command::new("ip")
                .args(["netns", "exec", &config.tiles.quic.interface])
                .args(["ip", "address", "show", "dev", &config.tiles.quic.interface])
                .output()
                .unwrap();
        ...
        let src_mac_address = if config.netns.enabled {
            Command::new("ip")
                .args(["netns", "exec", &config.tiles.quic.interface, "ip", "address", "show", "dev", &config.tiles.quic.interface,])
                .output()
                .unwrap()
        } else {
            Command::new("ip")
        ...
        for i in 0..config.tiles.verify.count as usize {
            ...
            if config.netns.enabled {
                run!(bin, "fd_xdp_ctl", ["--netns", &format!("/var/run/netns/{}", &config.tiles.quic.interface), "bind-xsk", xsk, name, config.tiles.quic.interface, i]);
            } else {
    ...
    fn undo(&mut self, config: &Config) {
        ...
        std::fs::remove_dir_all(&config.scratch_directory).unwrap();
    }
