...
/* peek_initial_state: Extracts the initial PoH state by peeking at the
   ...
   aligned at the first entry of a block, and that the entire entry
...
peek_initial_state( FILE *           shreds_file,
  ...
  /* Peek previous PoH state by looking at fragmented entry data.
     This is only valid if the first data batch of the block contains
     at least one entry header and if that first entry header is not
     fragmented across multiple shreds.  Our assumptions hold on current
     Solana mainnet constraints:
     Each block has 64 entries, each shred must have sizeof FD_SHRED_SZ,
     each shred must fill all the available data space. */

  ulong entry_hash_off = 8UL /* Data batch header size */
                       + offsetof( fd_entry_hdr_t, hash );

  /* Bounds check */
  FD_TEST( fd_shred_payload_sz( shred )>=(entry_hash_off+FD_SHA256_HASH_SZ) );
  /* Read entry count in first batch header -- Must have at least 1 entry. */
  FD_TEST( FD_LOAD( ulong, fd_shred_data_payload( shred ) )>0UL );

  /* Extract info */
  *tick = shred->slot << 5;
  memcpy( poh->state, fd_shred_data_payload( shred )+entry_hash_off, FD_SHA256_HASH_SZ );
...
main( int     argc,
  ...
  void * entry_mem = aligned_alloc( fd_entry_align(), footprint );
  FD_TEST( entry_mem );

  void * shentry = fd_entry_new( entry_mem, txn_max_cnt );
  FD_TEST( shentry );
  ...
  fd_entry_delete( fd_entry_leave( entry ) );
